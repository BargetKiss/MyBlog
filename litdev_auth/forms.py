...
class LitdevCreationForm(forms.ModelForm):
    ...
    username = forms.RegexField(max_length=30,regex = r'^[\w.@+-]+$',error_messages={
        'invalid':  u"该值只能包含字母、数字和字符@/./+/-/_",
        'required': u"用户名未填"
    })
    
    email = forms.EmailField(error_messages={
        'invalid':u"email 格式错误",
        'required':u"email未填"
    })
    ...
    def clean_username(self):
        ...
        try:
            LitdevUser._default_manage.get(username=username)
        except LitdevUser.DoesNotExist:
            return username
    ...
    def clean_email(self):
        ...
        #判断email用户是否存在
        try:
            LitdevUser._default_manage.get(email=email)
        except LitdevUser.DoesNotExist:
            return email
        raise forms.ValidationError(
    ...
    def save(self,commit=True):
        user = super(LitdevCreationForm,self).save(commit=True)
...
class LitdevPasswordResetForm(forms.Form):
    #错误信息
    error_messages = {
    ...
    def clean(self):
        ...
        if username and email:
            try:
                #self.user = LitdevUser.objects.get(username=username,email=email,is_active=True)
    ...
    def save(self,from_email=None,request=None,token_generator=default_token_generator):
        ...
        message = u"您收到这封信是因为你请求重置你在网站 %s 上的账户密码\n\n" % site_name +\
                  u"请访问该页面并输入新密码：\n\n" +\
        ...
        try:
            ...
            logger.error(u'[UserControl]用户重置密码邮件发送失败 %s/%s ,异常原因：%s '% (username,email,e))